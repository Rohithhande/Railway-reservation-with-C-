...
public:
    ...
    int offset; // File offset where the details are stored

    void display()
    {
        cout << "Train No.: " << tno << ", Offset: " << offset << endl;
    }
};
class Detail
{
...
void cancell();
void enquiry();
void modifyTrainDetails(int trainNumber);
void searchTrainByNumber(int trainNumber);
void addIndexEntry(int trainNumber, int offset);
int main(void)
{
	int ch;
...
{
    fstream indexFile;
    indexFile.open("index.txt", ios::out | ios::binary | ios::app);
    IndexEntry entry;
    entry.tno = trainNumber;
    entry.offset = offset;
    indexFile.write((char*)&entry, sizeof(IndexEntry));
    indexFile.close();
}
void searchTrainByNumber(int trainNumber) {
    fstream f;
...
void modifyTrainDetails(int trainNumber) {
    ...
    while (f.read((char*)&a, sizeof(a)))
    {
        ...
        {
            found = true;
            cout << "Enter new details for train with number " << trainNumber << ":" << endl;
            a.getDetail();
            offset = f.tellg() - sizeof(a);
            f.seekp(offset);
            f.write((char*)&a, sizeof(a));
            cout << "Train details modified successfully." << endl;
            break;
        }
    }
    f.close();
...
void database()
{
	...
	char c;
    fstream indexFile;
    indexFile.open("index.txt", ios::out | ios::binary);
    indexFile.close();
	 int trainNumber;
	...
	if(strcmp(pass,password)==0)
	{
		...
		do
		{
			...
			std::cout << "6. Search Train by Train Number\n";
            std::cout << "7. Modify Train Details by Train Number\n";
			...
			switch(ch)
			{
                ...
                a.getDetail();
                f.write((char*)&a, sizeof(a));
                addIndexEntry(a.tno, f.tellp() - sizeof(a)); // Updating the primary index
				...
				case 6:
                    ...
                    cin >> trainNumber;
                    searchTrainByNumber(trainNumber);
                    break;
                case 7:
...
void enquiry()
{
    ...
    {
        a.displaydetail();
        addIndexEntry(a.tno, offset); // Add the train number and offset to the primary index
        offset = f.tellg();
    }
    f.close();
}
